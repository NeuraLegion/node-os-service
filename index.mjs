...
const linuxStartStopScript = [
	...
	'		echo starting ##NAME##',
	'',
	'		cd "$workdir" && ##COMMAND## >/dev/null 2>&1 &',
	'',
	'		echo $! > "$pidfile"',
	'',
	'		while [ : ]; do',
	...
	if (os === 'win32') {
		...
		try {
			...
			if (error.code === 'ENOENT') {
				...
				throw new Error('stat(/usr/lib/systemd/system) failed', { cause: error });
			}
		}
	...
	if (os === 'win32') {
		...
		try {
			...
			if (error.code === 'ENOENT') {
				...
				throw new Error(`stat(${systemDir}) failed`, { cause: error });
			}
		}
...
async function disableAsync(name) {
	...
	if (os === 'win32') {
		...
		try {
			...
			// Exit code 2 means "service is not started"
			if (error.code !== 2) {
				throw new Error('net stop failed', { cause: error });
			}
		}
	} else if (os === 'darwin') {
		const root = join(rootdir, '/Library/LaunchAgents');
